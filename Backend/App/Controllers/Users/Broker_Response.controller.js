...
class BrokerResponses {
  async BrokerResponse(req, res) {
    ...
    try {
      ...
      return res.send({
        status: true,
        msg: "all broker response fatched successfully",
        data: findResponse,
      });
    } catch (err) {
...
const GetAllBrokerResponse1 = async (user_id, res) => {
  try {
    ...
    const FindUserAccessToken = await User.findOne({
      _id: objectId,
      TradingStatus: "on",
    });

    const FindUserBrokerResponse = await BrokerResponse.find({
      user_id: objectId,
      order_id: { $ne: "" },
    });
          ...
          try {
            ...
            const config = {
              ...
              headers: {
                Authorization:
                  "Bearer " +
                  FindUserAccessToken.demat_userid +
                  " " +
                  FindUserAccessToken.access_token,
            ...
            // console.log("response.data", response.data[0].Status);
            if (response.data[0]) {
