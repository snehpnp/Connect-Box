...
class GroupService {
    ...
    try {
      ...
      function checkStringValidity(grpService) {
        ...
        if (grpService.length < 5) {
          return res.send({
            status: false,
            msg: "Please Enter Group name long",
            data: [],
          });
        }
        ...
        if (
          grpService.substring(0, 3) !==
          grpService.substring(0, 3).toUpperCase()
        ) {
          return res.send({
            status: false,
            msg: "Please Enter Group Name starting 3 letter Capital",
            data: [],
          });
        }
        ...
        if (grpService.charAt(3) != "_") {
          return res.send({
            status: false,
            msg: "Please Enter Group  name _ is mandatory Emaple AAA_",
            data: [],
          });
        }
        if (
          maker_id_find.prifix_key != grpService.substring(0, 3).toUpperCase()
        ) {
          return res.send({
            status: false,
            msg: "Please Enter Group starting 3 leter is your Prefix Key letter",
            data: [],
          });
        }
        return true;
      ...
      if (!checkStringValidity(groupdetails.name)) {
        return res.send({
          status: false,
          msg: "Some Issue in Group Service",
          data: [],
        });
      }
      ...
      if (services_id.length > 50) {
        return res.send({
          status: false,
          msg: "You are Select Only 50 Services",
          data: groupServices,
        });
      }
  ...
  async Editgroupservice(req, res) {
    try {
      ...
      const strategy_check = await serviceGroupName.findOne({
        _id: GroupServices_Id,
      });
      if (!strategy_check) {
      ...
      // Check if the first three letters are capitalized
      if (
        groupdetails.name.substring(0, 3) !==
        groupdetails.name.substring(0, 3).toUpperCase()
      ) {
        return res.send({
          status: false,
          msg: "Please Enter Group Name starting 3 letter Capital",
      ...
      if (groupdetails.name.charAt(3) != "_") {
        return res.send({
          status: false,
          msg: "Please Enter Group name _ is mandatory",
          data: [],
        });
      }
      if (
      ...
      if (add_Group_services.length > 0) {
        add_Group_services.forEach(async (data) => {
          ...
          var find_user_service = await group_services.find({
  ...
  async DELETEGROUPSERVICES(req, res) {
    try {
      ...
      const groupServices_user = await group_services.find({
  ...
  async GetAllServicesName(req, res) {
    try {
      ...
      if (!data || data.length == 0 || data == "") {
        return res.send({
          status: false,
          msg: "Please Insert Coreect Data ",
          data: ServicesArr,
        });
      }
