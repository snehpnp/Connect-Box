...
var User_Wallet = db.User_Wallet;
...
class Users {
  ...
  async AddUser(req, res) {
    try {
      ...
      if (SubadminCheck.length == 0) {
        return res.send({
          status: false,
          msg: "Please Enter Correct Maker Id",
          data: [],
        });
      }
      ...
      try {
        ...
        var matchedStrategies = await Strategie_modal.find({
          _id: { $in: stgIds },
        }).select(
          "strategy strategy_demo_days security_fund_month security_fund_quarterly security_fund_half_early security_fund_early Service_Type fixed_amount_per_trade_early fixed_amount_per_trade_half_early fixed_amount_per_trade_quarterly fixed_amount_per_trade_month strategy_percentage researcher_id research_strategy_percentage purchase_type"
        );

        // Create an array of matched strategy IDs
        var matchedStrategyIds = matchedStrategies.map((strategy) =>
          strategy._id.toString()
        );

        // Find IDs that didn't match
        var unmatchedIds = Strategies.filter(
          (stg) => !matchedStrategyIds.includes(stg.id)
        );
      } catch (error) {
      ...
      if (
        !SubadminCheck[0].prifix_key ||
        SubadminCheck[0].prifix_key == "" ||
        SubadminCheck[0].prifix_key == null
      ) {
        return res.send({
          status: false,
      ...
      User_model.insertMany([
        {
          ...
          parent_id:
            SubadminCheck[0].Role == "SUBADMIN"
              ? SubadminCheck[0]._id
              : SubadminCheck[0].parent_id,
          parent_role:
            SubadminCheck[0].Role == "SUBADMIN"
              ? SubadminCheck[0].Role
              : SubadminCheck[0].parent_role,
          ...
          if (license_type == "0") {
            ...
            if (matchedStrategies.length > 0) {
              matchedStrategies.forEach((data) => {
                const matchedStrategy = Strategies.find(
                  (strat) => strat.id === data._id.toString()
                );
            ...
            if (matchedStrategies.length > 0) {
              matchedStrategies.forEach((data) => {
                ...
                UpdateDate = StartDate.setDate(
                  StartDate.getDate() + Number(data.strategy_demo_days)
                );
            ...
            if (matchedStrategies.length > 0) {
              matchedStrategies.forEach((data) => {
                ...
                const Researcher_charge_percentage =
                  Number(data.research_strategy_percentage || 0) / 100;
                const Researcher_charge1 =
                  Researcher_charge_percentage * Number(price_stg);

                const Admin_charge_percentage =
                  parseInt(SubadminCheck[0].strategy_Percentage) / 100;
                const Admin_charge1 =
                  Admin_charge_percentage * parseInt(price_stg);
                ...
                const strategy_transactionData = new strategy_transaction({
                  ...
                  Research_charge:
                    data.purchase_type != "monthlyPlan"
                      ? data.researcher_id
                        ? Researcher_charge1
                        : 0
                      : null,
  ...
  async UpdateUser(req, res) {
    try {
      ...
      const ParentData = await User_model.findOne({ _id: PID }).select(
      ...
      if (ParentData.subadmin_service_type == 1) {
        ...
        if (
          result &&
          result.length > 0 &&
          result[0].totalLicense !== undefined
        ) {
        ...
        if (
          stg_count &&
          stg_count.length > 0 &&
          stg_count[0].totalAdminCharge !== undefined
        ) {
      ...
      var add_startegy = [];
      await Promise.all(
        req.Strategies.map(async (strategy) => {
          if (
            !ExistStrategy.some(
              (existingStrategy) => existingStrategy.strategy_id == strategy.id
            )
          ) {
            if (req.license_type == "2") {
              add_startegy.push(strategy);
            } else {
              const Strategy_Details = await Strategie_modal.findOne({
                _id: strategy.id,
              }).select("strategy_name strategy_demo_days");
      ...
      ExistStrategy.forEach(function (strategy, index) {
        if (
          !req.Strategies.some(
            (existingStrategy) =>
              existingStrategy.id === strategy.strategy_id.toString()
          )
        ) {
      ...
      if (existingUsername.license_type != "2") {
        ...
        if (req.license_type == "0") {
          ...
          if (
            Exist_strategy.length > 0 &&
            existingUsername.license_type != "0"
          ) {
            Exist_strategy.forEach(async (data) => {
              ...
              const update_token = await strategy_client.updateOne(
                filter,
                update,
                { upsert: true }
              );
          ...
          if (add_startegy.length > 0) {
            add_startegy.forEach(async (data) => {
              ...
              if (data.plan_id == 1) {
                ...
                daysforstg = 6;
                trade_charge =
                  matchedStrategy.fixed_amount_per_trade_half_early;
              ...
              const User_strategy_client = new strategy_client({
                ...
                End_Date: EndDate1,
                uniqueUserStrategy:
                  existingUsername._id + "_" + matchedStrategy._id,
                trade_charge: trade_charge,
              });
              User_strategy_client.save();
              ...
              const Researcher_charge_percentage =
                Number(matchedStrategy.research_strategy_percentage || 0) / 100;
              const Researcher_charge1 =
                Researcher_charge_percentage * Number(price_stg);

              const Admin_charge_percentage =
                parseInt(ParentData.strategy_Percentage) / 100;
              const Admin_charge1 =
                Admin_charge_percentage * parseInt(price_stg);
              ...
              const strategy_transactionData = new strategy_transaction({
                ...
                Admin_charge: Admin_charge1,
                Research_charge:
                  matchedStrategy.purchase_type != "monthlyPlan"
                    ? matchedStrategy.researcher_id
                      ? Researcher_charge1
                      : 0
                    : null,
              });
        ...
        if (req.license_type == "2") {
          if (add_startegy.length > 0) {
            add_startegy.forEach(async (data) => {
              ...
              const Admin_charge_percentage =
                Number(ParentData.strategy_Percentage) / 100;
      ...
      var User_update = {
        ...
        Start_Date:
          existingUsername.license_type != 0 && req.license_type == 0
            ? StartDate1
            : null,
        End_Date:
          existingUsername.license_type != 0 && req.license_type == 0
  ...
  async GetAllUser(req, res) {
    try {
      ...
      const parent_role = await User_model.find({ _id: user_ID }).select(
        "Role"
      );
      var AdminMatch;
    ...
    try {
      ...
      if (parent_role[0].Role == "EMPLOYEE") {
        if (parent_role[0].subadmin_permissions.show_all_users == 1) {
          ...
          parent_role[0].subadmin_permissions.show_employee_users == 1
        ) {
          AdminMatch = { Role: "USER", employee_id: user_ID };
        } else {
  ...
  async GetUser(req, res) {
    try {
      ...
      const ClientGroupName = await groupService_User.find({
        user_id: user_ID,
      });
  ...
  async DeleteUser(req, res) {
    try {
      ...
      const user_Model_ckeck = await User_model.findOne({
        _id: id,
        Role: "USER",
      });
      if (!user_Model_ckeck) {
  ...
  async update_payment_status(req, res) {
    try {
      const { id, status, balance, type } = req.body;
      ...
      if (status == 1) {
        ...
        if (type === "withdrawal") {
          if (userDetail.Balance < balanceToAdd) {
            return res.json({
              status: false,
