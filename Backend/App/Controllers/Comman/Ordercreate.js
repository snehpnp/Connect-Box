...
class Ordercreate {
  ...
  async CreateOder(req, res) {
    ...
    try {
      ...
      res
        .status(500)
        .send({ status: false, data: error, msg: "Request not good" });
    }
  }
  ...
  async UpdateOrder(req, res) {
    try {
      ...
      if (type !== "monthlyPlan") {
        ...
        const existingCollaborator = await Stg_Collaborators.findOne(
          collaboratorFilter
        );
        ...
        await Stg_Collaborators.updateOne(
          collaboratorFilter,
          collaboratorUpdate,
          { upsert: true }
        );

        const updateCollaborator = await Stg_Collaborators.updateOne(
          collaboratorFilter,
          collaboratorUpdate,
          { upsert: true }
        );
      ...
      const updateStrategy = await researcher_strategy.updateOne(
        strategyFilter,
        strategyUpdate
      );
      ...
      const updateOrder = await strategy_Order_modal.updateOne(
        orderFilter,
        orderUpdate,
        { upsert: true }
      );
      ...
      res
        .status(500)
        .send({ status: false, data: error, msg: "Update failed" });
    }
  }
  ...
  async CreateOderStg(req, res) {
    try {
      ...
      const userDetail = await User_model.findOne({ _id: user_id });

      let UserTransection = await strategy_transaction.find({
      ...
      let RemaingBalance = userDetail.Balance - UsedBalance;


      if (parseFloat(amount) / 100 > RemaingBalance) {
        return res.send({
          status: false,
      ...
      const matchedStrategy = await strategy_model
      .findOne({ _id: strategy_id })
      .select(
    ...
    const ParentData = await User_model.findOne({
      _id: new ObjectId(matchedStrategy.maker_id),
    }).select("Balance subadmin_service_type strategy_Percentage");
    ...
    const currentDate = new Date();
    const start_date_2days = dateTime
      .create(currentDate)
      .format("Y-m-d H:M:S");
    ...
    const UpdateDate = new Date(
      StartDate.setMonth(StartDate.getMonth() + parseInt(daysforstg))
    );
    ...
    // Update strategy_Order_modal collection
    const filter = { _id: order_data._id };
    const update = { $set: req.body };
    const update_token = await User_strategy_Order.updateOne(filter, update, {
      upsert: true,
    });

   return res.send({ status: true, data: update_token, msg: "Update successful" });;
    } catch (error) {
      console.error("Error creating order:", error);
      res
        .status(500)
  ...
  async Createorderfreepal(req, res) {
    try {
      const { strategy_id, user_id, type, id } = req.body;
