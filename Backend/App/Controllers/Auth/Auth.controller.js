...
class Auth {
    ...
    async login(req, res) {
        try {
            ...
            // JWT TOKEN CREATE
            const token = jwt.sign({ id: EmailCheck._id }, SECRET_KEY, { expiresIn: 36000 }); // 10 hours

            // Logout from other device
            if (device == "APP" && EmailCheck.web_login_token) {
                io.to(EmailCheck.web_login_token).emit('logout', { message: 'Logged out due to login from another device' });
            } else if (device == "WEB" && EmailCheck.web_login_token) {
                io.to(EmailCheck.web_login_token).emit('logout', { message: 'Logged out due to login from another device' });
            }

            // Update token based on the device
            let token_query = {};
            if (device == "APP") {
                token_query = { app_login_token: token, AppLoginStatus: 1, WebLoginStatus: 0 };
            } else {
                token_query = { web_login_token: token, WebLoginStatus: 1, AppLoginStatus: 0 };
            }

            await User.findByIdAndUpdate(EmailCheck._id, token_query, { new: true });
            ...
            return res.send({ status: true, msg: "Login Successfully", data: msg });
        } catch (error) {
    ...
    async logoutUser(req, res) {
        try {
            ...
            if (Device.toUpperCase() == "APP") {
                ...
                addData["app_login_token"] = '';
            } else if (Device.toUpperCase() == "WEB") {
                addData["WebLoginStatus"] = 0;
                addData["web_login_token"] = '';
            }

            await User.findByIdAndUpdate(EmailCheck._id, addData);
            ...
            return res.send({ status: true, msg: "Logout Successfully", data: [] });
        } catch (error) {
