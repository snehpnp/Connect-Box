...
import { No_Negetive_Input_regex, Yearly_plan_regex, Halfyearly_plan_regex, Quaterly_plan_regex, Monthly_plan_regex ,Stg_regex} from "../../../../Utils/Common_regex"
...
const EditStrategy = () => {
  ...
  const isValidName = (stg) => {
    return Stg_regex(stg)
  }
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      else if (!isValidName(values.strategyname) && formik.touched.strategyname) {
        errors.strategyname = valid_err.INVALID_ERROR;
      }
    ...
    onSubmit: async (values) => {
      ...
      const req = {
        _id: id,
        'strategy_name': values.strategyname.replace(/\s+/g, '').trim(),
