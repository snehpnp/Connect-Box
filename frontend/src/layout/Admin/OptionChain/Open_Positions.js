...
const TradeHistory = () => {
   ...
   // let socketGetNotification = io("http://193.239.237.139:7700");
    ...
    /////----------------ADD SOCKET CODE GET EXIT TRADE--------------////////////
     const [socketSignal, setSocketSignal] = useState(null);
    // const connect_socket = () => {
    //    const urls = 'http://193.239.237.139:7700';
    //    // const urls = 'http://app.aitech.market/backend/';
    ...
    // useEffect(() => {
    //     connect_socket()
    //   }, []);  ----------> this logic implement company details get Api  set

   

    const GetNotficationExitTrade = () => {
    
    if (socketSignal) {
    // Set up event listener for receiving messages
    socketSignal.on('TEST_TRUST', (message) => {
      // alert("okkk")
    ...
    useEffect(() => {
      //  GetNotficationExitTrade()
      }, [socketSignal]); // Re-run effect when socketSignal changes

     
      /////----------------END ADD SOCKET CODE GET EXIT TRADE--------------////////////
        ...
        await dispatch(GET_COMPANY_INFOS())
            ...
            .then((response) => {
               ...
               // console.log(" res version",res.version)
                ...
                //const socket = socketIOClient(res.version);
...
const filteredArray2 = tradeHistoryAllData.data.filter(item => selected1.some(obj => obj._id === item._id));
    ...
    const forMCXandCurrencyMarketTrade = () => {
        ...
        if (MarketOpenToday) {
            ...
            EQcutoffTimeIST.setHours(23, 30, 0, 0);
