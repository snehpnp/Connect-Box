...
const AllSubadmin = () => {
    ...
    const [licenceInput, setLicenceInput] = useState(0);
    const [errorMessage, setErrorMessage] = useState('');
    const [addLicence, setAddLicence] = useState('');
    const [remainingLicenses, setRemainingLicenses] = useState('');


    console.log("remainingLicenses :", remainingLicenses)
        ...
        await dispatch(Get_Dashboard_Count(user_token))
            ...
            .then((response) => {
                if (response.status) {
                    ...
                    setRemainingLicenses(response.totalCount.remaining_licence)
    ...
    const formik = useFormik({
        ...
        validate: (values) => {
            ...
            if (remainingLicenses < addLicence) {
                errors.remaininglicense = `Please Add license less then ${remainingLicenses}`
            }
            if (errorMessage.length) {
                errors.error_massage = 'Please enter only numbers'
              }
        ...
        onSubmit: async (values) => {
            ...
            const req = {
                ...
                "Subadmin_permision_data": {
                    ...
                    "licence": values.licence ? licenceInput : values.all ? licenceInput : '0',
                    "add_licence": values.licence ? addLicence : values.all ? addLicence : '0'
                }
            }
    ...
    useEffect(() => {
        if (UserData.data.data !== undefined && UserData.data.data.length > 0) {
            ...
            setLicenceInput(UserData.data.data[0].subadmin_permissions[0].licence)
        }
    }, [UserData.data]);
                                            ...
                                            <div className='col-lg-12'>
                                                <div className="row">
                                                    <div className={`col-lg-3`}>
                                                        <h6 className='fw-bold mt-3 text-decoration-none'>Add More Licence</h6>
                                                        <div className={`col-lg-12 mb-4`}>
                                                            <div className="form-group">
                                                                <div className="col-sm-12">
                                                                    <input
                                                                        type='text'
                                                                        className={`rounded p-2 ${errorMessage ? 'is-invalid' : ''}`}
                                                                        value={addLicence}
                                                                        onChange={(e) => { hendleLicenceChange(e) }}
                                                                        placeholder='Add More Licence'
                                                                    />
                                                                    
                                                                    {formik.errors.error_massage && (
                                                                        <div style={{ color: "red" }}>
                                                                            {formik.errors.error_massage}
                                                                        </div>
                                                                    )}
                                                                    {formik.errors.remaininglicense && (
                                                                        <div style={{ color: "red" }}>
                                                                            {formik.errors.remaininglicense}
                                                                        </div>
                                                                    )}
                                                    ...
                                                    <div className={`col-lg-3`}>
                                                        <h6 className='fw-bold mt-3 text-decoration-none'>Previous Given Licence</h6>
