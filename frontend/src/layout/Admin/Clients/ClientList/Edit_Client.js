...
const AddClient = () => {
  ...
  // COUNT NUMBER OF CHECKED STRATEGY
  const checkedStrategiesCount = selectedStrategies.filter(strategy => strategy.checked);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const req = {
        ...
        "Editor_role": Role,
        "device": check_Device(),
        "count_strategy_select": selectedValue,
        "multiple_strategy_select": values.multiple_strategy_select === false ? '0' : '1'
      }

      if (selectedValue== 0 ||  selectedValue > checkedStrategiesCount.length) {
        alert("Please select limit is less then selected strategy")
        return
      }
      else {
  ...
  console.log("UserData", UserData.data.data && UserData.data.data[0].count_strategy_select)
  useEffect(() => {
    console.log("UserDatainside", UserData.data.data !== undefined && UserData.data.data[0].count_strategy_select)
    ...
    useEffect(() => {
      setSelectedValue( UserData.data.data !== undefined &&  UserData.data.data[0].count_strategy_select)
    }, [UserData.data])
  ...
  const fields =
    [
      ...
      //   label: 'Limit',
      //   type: 'select',
      //   options: Array.from({ length: checkedStrategiesCount }, (_, i) => ({ label: (i + 1).toString(), value: i + 1 })),
  ...
  const handleChangeSelectLimit = (event) => {
          ...
          additional_field={
            <>
              ...
              {selectedStrategies.map((strategy) => (
                <div className={`col-lg-4 mt-2`} key={strategy.id}>
                  <div className="row ">
                    <div className="col-lg-12 ">
              ...
              <div className={`col-sm-6 mb-4`}>
                <div className="form-group">
                  ...
                  <div className="col-sm-12">
                    <select className="form-control" value={selectedValue} onChange={handleChangeSelectLimit}>
                      <option value='0'>Please Select Limit</option>
                      {checkedStrategiesCount.map((option, index) => (
