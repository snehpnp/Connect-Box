...
console.log('selectedValue :', selectedValue)
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (values && values.limit > selectedStrategies.length && formik.touched.limit) {
        errors.limit = valid_err.LIMIT_STRATEGY_ERROR;
      }
    ...
    if(selectedValue==0 ||  selectedValue > selectedStrategies.length){
  ...
  const getGroupeServics = async () => {
    await dispatch(Get_Service_By_Group_Id({ _id: "653367df827128bcdfae41c8" })).unwrap()
      .then((response) => {
        if (response.status) {
          setGetServices({
            loading: false,
          ...
          additional_field={
            <>
              ...
              {AllStrategy.data.map((strategy) => (
                <div className={`col-lg-4 mt-2`} key={strategy._id}>
                  <div className="row ">
                    <div className="col-lg-12 ">
              ...
              <div className={`col-sm-6 mb-4`}>
                <div className="form-group">
                  ...
                  <select className="form-control" value={selectedValue} onChange={handleChangeSelectLimit}>
                    ...
                    {selectedStrategies.map((option, index) => (
