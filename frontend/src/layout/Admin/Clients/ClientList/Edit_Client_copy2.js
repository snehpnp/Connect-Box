...
const AddClient = () => {
  ...
  const checkedStrategiesCount = selectedStrategies.filter(strategy => strategy.checked).length;
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if(values && values.limit > checkedStrategiesCount && formik.touched.limit){
    ...
    onSubmit: async (values) => {
      const req = {
        ...
        "Editor_role": Role,
        "device": check_Device(),
        "count_strategy_select": values.limit,
  ...
  useEffect(() => {
    ...
    formik.setFieldValue('limit', UserData.data.data !== undefined && UserData.data.data[0].count_strategy_select);
