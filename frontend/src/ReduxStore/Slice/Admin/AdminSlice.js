...
import { ALL_SERVICES, ALL_CATAGORY, PAYMENT_GATEWAY_KEY, GetPaymentDetails , SERVICE_BY_CATAGORY, ALL_GROUP_SERVICES, ALL_EXPIRED_CLIENTS, ALL_CLIENTS, GET_COMPANY_LOGO, GET_COMPANY_INFO, GET_ALL_TRADINGSTATUS, GO_TO_DASHBOARD, UPDATE_USERACTIVE_STATUS, DELETE_USERAND_ALLSERVICES, FIND_ONE_USER, UPDATE_USER,UPDATE_PAYMENT_AMOUNT } from "../../../Service/admin.service";
...
// Payment Gateway key
export const PaymentGatewayKey = createAsyncThunk("api/UpdatePaymentGatewayKey", async (data) => {
    try {
   
        const res = await PAYMENT_GATEWAY_KEY(data);
...
// Upadte Payment Amount
export const UpdatePaymentAmount = createAsyncThunk("api/UpdatePaymentAmount", async (data) => {
    try {
   
        const res = await UPDATE_PAYMENT_AMOUNT(data);
...
export const GET_PAYMENT_DETAILS = createAsyncThunk("api/UserPaymentDetails", async (data) => {
    try {
...
const AdminSlice = createSlice({
    ...
    extraReducers: {
        ...
        [GET_PAYMENT_DETAILS.fulfilled]: (state, { payload }) => {
            // state.isLoading = false;
            return { ...state, paymentDetails: payload, isLoading: false };
        },
        [PaymentGatewayKey.fulfilled]: (state, { payload }) => {
            // state.isLoading = false;
            return { ...state, PaymentGateway: payload, isLoading: false };
        },
        [UpdatePaymentAmount.fulfilled]: (state, { payload }) => {
            // state.isLoading = false;
            return { ...state, UpdatePaymentAmount: payload, isLoading: false };
        }
