    ...
    const columns = [
        {
            ...
            formatter: (cell, row, rowIndex) => (
                ...
                >
                    <button
                        value="LE"
                        className={`button_BUY button1 button_call_buy_${row.call_token}`}
                        onClick={(e) => {
                    ...
                    <button
                        value="SE"
                        className={`button_sell button2 button_call_sell_${row.call_token}`}
                        onClick={(e) => {
        ...
        },
        {
            ...
            formatter: (cell, row, rowIndex) => (
                ...
                >
                    <button
                        value="LE"
                        className={`button_BUY button1 button_put_buy_${row.put_token}`}
                    ...
                    <button
                        value="SE"
                        className={`button_sell button2 button_put_sell_${row.put_token}`}
            ...
            axios.request(config)
                .then((response) => {
                    ...
                    if(user_role == "ADMIN"){
                        ...
                        setTimeout(() => {
                          navigate("/subadmin/openposition")
                         }, 1000);
                       }else{
                ...
                if (response.status) {
                    ...
                    if(user_role == "ADMIN"){
                        ...
                        if (getPermissions && getPermissions.strategy !== undefined) {
                            let abc = response.data && response.data.filter(item => getPermissions.strategy.includes(item._id))
                            if (abc.length > 0) {
                                setAllStrategyName({
                                loading: false,
                                data: abc
                ...
                All_Symbols.loading ? <Loader /> :
                    <>
                        <Theme_Content Page_title="Option Chain" button_status={false}>
                            ...
                            <div className='option-chain mt-2' style={{ maxHeight: '400px', overflowY: 'auto' }}>
