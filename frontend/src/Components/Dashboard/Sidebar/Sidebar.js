...
import { Signal, Users, Wrench, Link2, Frame, CandlestickChart, Activity, WalletCards, IndianRupee, HelpingHand, FolderClock, LayoutDashboard, Building2, Copyright, Repeat2, ArrowRightLeft, ScatterChart, Boxes, Rocket, Paintbrush, Vote, Info } from 'lucide-react';
...
const Sidebar = ({ ShowSidebar }) => {
    ...
    return (
        <div>
            ...
            <div className="deznav pt-5" style={{ overflowX: 'auto', maxHeight: '80vh' }} >
                <div className="deznav-scroll">
                    <div className=" dropdown header-bx text-center">
                        <a className="nav-link header-profile2 position-relative" href="javascript:void(0);" role="button" data-bs-toggle="dropdown">
                            <div className="header-img position-relative">
                                <img src="/assets/images/demo/avt.png" alt="header-img" />
                            </div>
                            <div className="header-content">
                                <span className="font-w400">Welcome</span>
                            ...
                            gotodashboard != null ? user_role_goTo === "USER" ? Client && Client.map((item) => {
                                return <>
                                    ...
                                    <li className={`${location.pathname === item.route && item.route ? 'mm-active' : ""}`}>
                                        {item.Data.length > 0 ? <>
                                            <Link
                                                className="has-arrow font-weight-bold "
                                        ...
                                        <ul aria-expanded="false">
                                            {item.Data.length > 0 ?
                                                item.Data.map((nested_item) => {
                                                    return <>
                                                        <li className={`${location.pathname === item.route && item.route ? 'mm-active' : ""}`}>
                                                            <Link to={nested_item.route} style={{ textDecoration: 'none' }}>{nested_item.name}</Link>
                                                        </li>
                                                    </>
                                                })
                                                : ""}
                                ...
                                roles === 'ADMIN' ? admin_sidebar && admin_sidebar.map((item) => {
                                    ...
                                    roles === 'SUBADMIN' ? sub_admin_sidebar && sub_admin_sidebar.map((item) => {
                                        ...
                                        console.log("getPermissions ", item.id, item.route)

                                        console.log("item.Data ", item.Data)

                                        if (item.id == 8 || item.id == 9) {
                                            console.log("location.pathname ", location.pathname)
                                            if ((item.id == 8 && getPermissions && getPermissions.optionchain === 0) || (item.id == 9 && getPermissions && getPermissions.makestrategy === 0)) {
...
const IconComponent = ({ icon }) => {
    ...
    const renderIcon = () => {
        switch (icon) {
            ...
            case 'IndianRupee':
                return <IndianRupee className='me-3' />;
            case 'Building2':
