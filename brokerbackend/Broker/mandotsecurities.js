    ...
    if (token != 0) {
      if (type == "LE" || type == "SE") {
        const requestPromises = AllClientData.map(async (item) => {
          item.postdata.exchangeInstrumentID = parseInt(
            token[0].instrument_token
          );
          ...
          axios(config)
              ...
              if (response.data.type == "success") {
                ...
                if (response.data.result.positionList.length > 0) {
                  const Exist_entry_order =
                    response.data.result.positionList.find(
                      (item1) =>
                        item1.ExchangeInstrumentId ==
                        parseInt(token[0].instrument_token)
                    );

                  if (Exist_entry_order != undefined) {
                    const possition_qty =
                      parseInt(Exist_entry_order.OpenBuyQuantity) -
                      parseInt(Exist_entry_order.OpenSellQuantity);
            ...
            .catch(async (error) => {
              fs.appendFile(
                ...
                "TIME " +
                  new Date() +
                  " MANDOT POSITION DATA ERROR CATCH - " +
                  item.UserName +
                  " ERROR - " +
...
const EntryPlaceOrder = async (item, filePath, signals, signal_req) => {
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " MANDOT BEFORE PLACE ORDER USER ENTRY- " +
      item.UserName +
      " REQUEST -" +
  ...
  axios(config)
    .then(async (response) => {
      ...
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " MANDOT AFTER PLACE ORDER USER ENTRY - " +
          item.UserName +
          " RESPONSE -" +
    ...
    .catch(async (error) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " MANDOT AFTER PLACE ORDER CATCH ENTRY - " +
          item.UserName +
          " ERROR -" +
          JSON.stringify(error) +
          "\n",
...
const ExitPlaceOrder = async (
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " MANDOT BEFORE PLACE ORDER USER EXIT- " +
      item.UserName +
      " REQUEST -" +
      JSON.stringify(item.postdata) +
      "\n",
  ...
  axios(config)
    .then(async (response) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " MANDOT AFTER PLACE ORDER USER EXIT- " +
          item.UserName +
          " RESPONSE -" +
          JSON.stringify(response.data) +
          "\n",
    ...
    .catch(async (error) => {
      fs.appendFile(
        ...
        "TIME " +
          new Date() +
          " MANDOT AFTER PLACE ORDER USER EXIT CATCH- " +
          item.UserName +
          " RESPONSE -" +
          JSON.stringify(error) +
          "\n",
...
const EntryPlaceOrderSemiAuto = async (item, filePath, signals, signal_req) => {
  ...
  fs.appendFile(
    ...
    "TIME " +
      new Date() +
      " MANDOT SECURITIES BEFORE PLACE ORDER INSER SEMI AUTO ENTRY- " +
      item.UserName +
      " REQUEST -" +
      JSON.stringify(item.postdata) +
      "\n",
