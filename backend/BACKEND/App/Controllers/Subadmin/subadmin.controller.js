...
class Subadmin {
    ...
    async AddSubadmin(req, res) {
        try {
            ...
            const objectIdArray = Subadmin_permision_data.strategy.map(value => new ObjectId(value));
          
           console.log("license -- ",Subadmin_permision_data.licence)

           const existUserStartegy = await Subadmin_Permission.aggregate([
               {
                $match: {
                  $and: [
                    { strategy: { $in: objectIdArray } },
                  ]
                }
               }
            ...
            if (existUserStartegy.length > 0) {
                return res.send({ status: false, msg: 'Strategy Already exists in other Sub Admin', data: [] });
            }
            ...
            const User = new User_model({
                ...
                parent_id: parent_id,
                Is_First_login: "1",
                licence:Number(Subadmin_permision_data.licence)
                    ...
                    const SuperadminInfo = SubadminPermision.save()
                        .then(async (data) => {
                            ...
                            const count_licenses_add = new count_licenses({
                                ...
                                license: Number(Subadmin_permision_data.licence),
                                Role:"SUBADMIN",
                                admin_id:parent_id
                              });
    ...
    async EditSubadmin(req, res) {
        try {
           ...
           const existUserStartegy = await Subadmin_Permission.aggregate([
            ...
            if(Number(Subadmin_permision_data.add_licence) > 0){
                Total_licence = Number(Subadmin_permision_data.add_licence)  + Number(Subadmin_permision_data.licence)
                const count_licenses_add = new count_licenses({
                    user_id: existingUsername._id,
                    license: Number(Subadmin_permision_data.add_licence),
                    Role:"SUBADMIN",
                    admin_id:parent_id
                  });
                  count_licenses_add.save();
            ...
            console.log("Total_licence",Total_licence)
            ...
            const User = {
                ...
                Otp: password,
                Role: Role.toUpperCase(),
                licence:Number(Total_licence)


            };
            let subadminUpdate = await User_model.findByIdAndUpdate(existingUsername._id, User)

            const Subadmin_Permission_data = await Subadmin_Permission.find({_id:existingUsername._id});
            ...
            var SubadminPermision = {
                ...
                optionchain: Subadmin_permision_data.optionchain,
                makestrategy: Subadmin_permision_data.makestrategy,
                licence:Total_licence
            }


            console.log("SubadminPermision cpp:", SubadminPermision)
