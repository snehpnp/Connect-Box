...
class Employee {
  ...
  async AddEmployee(req, res) {
    try {
      ...
      // console.log("multiple_strategy_select ",typeof multiple_strategy_select)
      // console.log("count_strategy_select ",count_strategy_select)
      ...
      User_model.insertMany([
        {
          ...
          count_strategy_select: count_strategy_select,
  ...
  async UpdateUser(req, res) {
    try {
      ...
      if (
        ...
        var client_services_exist_data = await client_services.find({
          user_id: existingUsername._id,
        });

        client_services_exist_data.forEach(async(item) => {
          // console.log('count_strategy_select' ,req.count_strategy_select)
          console.log('item startegy id' ,item.strategy_id)
          
          if(parseInt(req.count_strategy_select) < item.strategy_id.length){

            //console.log('item startegy id' ,item.strategy_id)

            const remain_strategy = item.strategy_id.slice(0,parseInt(req.count_strategy_select));
           console.log('item  remain_strategy' ,remain_strategy)
            
            var update_services = await client_services.updateMany(
              { _id: item._id },
              { $set: { strategy_id: remain_strategy } }
            );
            ...
            if(req.multiple_strategy_select == "0"){
             ...
             var update_services = await client_services.updateMany(
              { _id: item._id },
              { $set: { strategy_id: item.strategy_id[0] } }
            );
            }
          
        });

        //console.log("client_services_exist_data",client_services_exist_data)
        ...
        var User_update = {
          ...
          count_strategy_select: req.count_strategy_select,
  ...
  async DeleteUser(req, res) {
    try {
      ...
      var Subadmin_Permission_delete = await Subadmin_Permission.deleteMany({
        user_id: get_user[0]._id,
      });
  ...
  async GetclientKey(req, res) {
    try {
      ...
      if (!Client_key) {
        return res.sned({ status: false, msg: 'Client Not exists', data: [] });
      }
