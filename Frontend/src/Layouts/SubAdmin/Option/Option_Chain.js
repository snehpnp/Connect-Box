...
import {
  ...
  Get_Symbol_Expiry,
  Get_Company_Infos,
  Get_All_Strategy_for_Client,
} from "../../../ReduxStore/Slice/Subadmin/OptionChainSlice";
...
function Option_Chain() {
  ...
  const columns = [
    {
      ...
      style: (cell, row) =>
        ...
        parseInt(OptionChainData.data[16].strike_price)
  ...
  const symbols = async () => {
    ...
    await dispatch(Get_Option_Symbols(data))
      .unwrap()
  ...
  const getSymbolsExpriy = async () => {
    ...
    await dispatch(Get_Symbol_Expiry(data))
      .unwrap()
      .then((response) => {
        if (response.status) {
          ...
          if (response.tk && response.lp !== undefined) {
            function formatVolume(volume) {
              return volume >= 1e7
                ...
                : volume >= 1e5
                ? (volume / 1e5).toFixed(2) + " Lakh"
                : volume;
            }

            $(".Call_volume_" + response.tk).html(
              formatVolume(old_Call_volume_)
            );
    ...
    ExecuteTradeData.data &&
      ExecuteTradeData.data.map((item) => {
        ...
        let req = `DTime:${currentTimestamp}|Symbol:${symbol && symbol}|TType:${
          ...
          UserDetails && UserDetails[0].client_key
        }|TradeType:OPTION_CHAIN|ExitStatus:OPTION CHAIN|Demo:demo`;
                        ...
                        All_Symbols_Expiry.data.map((item) => {
                          return (
                            <option value={item.uniqueExpiryValues}>
                              {get_three_digit_month(item.expiryDate)}
                            </option>
                          );
                        })}
                    </select>
